...
import { generateTypeImports, formatImports, sanitizeIdentifier, toCamelCase, escapeStringLiteral, parseTypeFromTypeDef, formatJSDocComment } from './utils.js';
...
${eventMethods}
    ...
    if (events.length > 0) {
      ...
      events.forEach((event) => {
        const topicParams = event.params().filter(
          (param) => param.location().value === types.ScSpecEventParamLocationV0.scSpecEventParamLocationTopicList().value
        );
  ...
  generateEventFilterMethod(event, eventIndex) {
    ...
    const fields = topicParams.map((param) => {
      ...
      const fieldName = /^[a-zA-Z_$][a-zA-Z0-9_$]*$/.test(rawParamName) ? rawParamName : `"${escapeStringLiteral(rawParamName)}"`;
